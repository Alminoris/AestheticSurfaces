...
public class PathBlock extends Block
{
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit)
    {
        ...
        {
            String woodName = Registry.ITEM.getId(stack.getItem()).getPath().replace("_parquet_", "-").split("-")[1];
            String typeName = Registry.ITEM.getId(stack.getItem()).getPath().replace("_parquet_", "-").split("-")[0];
